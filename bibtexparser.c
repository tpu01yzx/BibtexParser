...
int main(int argc, char *argv[])
{
	...
	fi = -1; isOutputplain = false; isOnlyRegular = false; isUppercaseKey = false;
	for(i=1;i<argc;i++){
		if(strcmp(argv[i], "-O") == 0 || strcmp(argv[i], "--output") == 0) {
			...
			isOutputplain = true;
		} else if(strcmp(argv[i], "-U") == 0 ||strcmp(argv[i], "--uppercasekey") == 0) {
			isUppercaseKey = true;
		} else if(argv[i][0] == '-') {			
			printf("The only available options are: \n"
				TABSTR"-O --output if this option is not given, then stdout is used.\n"
				TABSTR"-U --uppercasekey default is false. If this option is set, then output citation keys as uppercase.\n"
			...
			if(be->type >= BTE_REGULAR) {
				...
				if(be->key) {
					if(isUppercaseKey) {
						...
						bib_tolwr(be->key, be->key, strlen(be->key));
					}
					printf("%s\n", be->key);
